...
const AdminPage = () => {
    ...
    const handlePublish = async () => {
        try {
            await set(ref(database, 'quizStatus'), true); // Set quiz status to true
            setPublished(true);
    ...
    const handleStop = async () => {
        try {
            await set(ref(database, 'quizStatus'), false); // Set quiz status to false
            setPublished(false);
    ...
    const handleAddQuestion = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                fetchQuestions(); // Refresh questions after adding
            } else {
        ...
        <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-r from-purple-400 to-indigo-500">
            ...
            <div className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 w-full max-w-lg">
                ...
                <div className="flex justify-between">
                    ...
                    <Button onPress={handlePublish} variant='ghost' >Publish</Button>
                    <Button onPress={handleStop} variant='ghost' >Stop</Button>
                </div>
            </div>
            <ToastContainer />
        </div>
